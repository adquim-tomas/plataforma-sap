...
export function InterempresaPanel() {
  ...
  const runPreview = async () => {
    ...
    try {
      const preview = await interempresaPreview(fechaMin, fechaMax)
      setPhase({ kind: "preview", preview })
    } catch (err) {
      setPhase({ kind: "failed", message: errMessage(err), recoverable: true })
    }
  }
  ...
  const runConfirm = async () => {
    ...
    try {
      const result = await interempresaRun(fechaMin, fechaMax)
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Rango de fechas — siempre visible salvo en done */}
      {phase.kind !== "done" && (
        <div className="flex flex-wrap items-end gap-4 border border-border bg-elev px-4 py-3">
      ...
      {phase.kind === "running" && (
        <div className="flex h-24 flex-col items-center justify-center gap-2 border border-dashed border-border-strong bg-elev">
          <div className="flex items-center gap-2">
            ...
            <Label className="text-foreground">cargando en Adgreen</Label>
          </div>
        </div>
      )}
