...
class CrearDesdeFolioHandler(BaseUploadHandler[CrearDesdeFolioRow]):
    ...
    async def sync_row(self, sap: SAPClient, row: CrearDesdeFolioRow) -> None:
        # Las validaciones de negocio (folio existe, no ambiguo, CardCode
        # consistente, hay líneas pendientes) requieren la misma respuesta
        # SAP que el POST — se hacen dentro del service para no duplicar la
        # llamada GET.
